...
- Capture: `cpal` in the client (the headless `goatsd` only relays).
- Codec: `libopus_sys` (static build, no system libopus) with a thin safe wrapper
  in one module; ~24 kbps, 20 ms frames (960 samples at 48 kHz mono).
