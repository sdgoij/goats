...
| Nothing else, ideally | the level, the mesh, the uniforms, the shader, the reflection, the wake and the splashes all run on the surface as it stands today -- and with the fill deleted (M20d″) a rebuild is one `O(cells)` scan for the window's lowest ground **plus the mesh and the bake over the rectangles the ground actually changed in**, which is what a crater's healing steps cost now (~1 ms, against ~170 whole-grid before it was measured -- `PERF.md`, *What a crater costs in the frame*) |
